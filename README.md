...
- **Sections are independent buckets** — built-in **Incomplete + Completed** are the *default* home (a goal with no user section auto-sorts between them on completion), plus user-defined sections in the middle band. A user section keeps its own goals: completing one **stays in place** (sinking to the bottom of the section as a ✓ checklist item) rather than jumping to Completed — unless you right-click the section header → **Auto-archive completed**, which graduates that section's finished goals out to the Completed list as they're done. Each section can be renamed, recolored, reordered, and right-clicked for bulk operations.
