...
- **Prereq seeding** — adding a quest/diary/boss goal that has its own requirements offers "Add Goal with Requirements" which recursively seeds the whole AND-linked prereq tree (skills, child quests, item requirements, account metrics, boss-kill prereqs, and OR-alternatives where defined). You can also seed requirements onto an **existing** quest goal after the fact: right-click it → **Add requirements to this section** → *Incomplete only* (just what you still need) or *All* (the whole tree, kept inline as cards so met requirements show as ✓ rather than archiving away).
