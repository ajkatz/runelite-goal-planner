...
/**
 * Encapsulates relation (requirement/dependency) methods extracted from {@link GoalTrackerApiImpl}.
...
class RelationService
{
	...
	RelationService(GoalTrackerApiImpl api)
	{
		this.api = api;
	}
	...
	GoalTrackerInternalApi.FindOrCreateResult findOrCreateRequirement(Goal template, String preferredSectionId)
	{
		...
		if (existing != null)
		{
			return new GoalTrackerInternalApi.FindOrCreateResult(existing.getId(), false);
		}
		...
		return ok ? new GoalTrackerInternalApi.FindOrCreateResult(seedId, true) : null;
	}
		...
		net.runelite.api.Quest quest)
	{
		...
		return com.goaltracker.data.QuestRequirementResolver.resolve(quest, api.client);
	}
}
