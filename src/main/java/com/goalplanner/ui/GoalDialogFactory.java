...
class GoalDialogFactory
{
	...
	void showAddGoalDialog(String preferredSectionId)
	{
		...
		metricCombo.setRenderer(new DefaultListCellRenderer()
		{
			...
			{
				super.getListCellRendererComponent(list, value, index, isSelected, cellHasFocus);
				if (value instanceof com.goalplanner.model.AccountMetric)
				{
					setText(((com.goalplanner.model.AccountMetric) value).getDisplayName());
				}
				return this;
			}
		});
		...
		// Boss combo
		String[] bossNames = com.goalplanner.data.BossKillData.getBossNames();
		...
		// CA tier dropdown
		String[] caTierOptions = new String[com.goalplanner.model.AccountMetric.CA_TIER_NAMES.length + 1];
		caTierOptions[0] = "-- Select tier --";
		System.arraycopy(com.goalplanner.model.AccountMetric.CA_TIER_NAMES, 0, caTierOptions, 1,
			com.goalplanner.model.AccountMetric.CA_TIER_NAMES.length);
		...
		caTierCombo.addActionListener(e -> {
			...
			if (idx > 0)
			{
				accountTargetField.setText(
					String.valueOf(com.goalplanner.model.AccountMetric.CA_TIER_VALUES[idx - 1]));
			}
		});
		...
		System.arraycopy(com.goalplanner.model.AccountMetric.LEAGUE_TIER_NAMES, 0, leagueTierOptions, 1,
			com.goalplanner.model.AccountMetric.LEAGUE_TIER_NAMES.length);
		...
		leagueTierCombo.addActionListener(e -> {
			...
			{
				accountTargetField.setText(
					String.valueOf(com.goalplanner.model.AccountMetric.LEAGUE_TIER_VALUES[idx - 1]));
			}
		});
		...
		System.arraycopy(com.goalplanner.model.AccountMetric.LEAGUE_AREA_NAMES, 0, leagueAreaOptions, 1,
			com.goalplanner.model.AccountMetric.LEAGUE_AREA_NAMES.length);
		...
		leagueAreaCombo.addActionListener(e -> {
			...
			{
				accountTargetField.setText(
					String.valueOf(com.goalplanner.model.AccountMetric.LEAGUE_AREA_VALUES[idx - 1]));
			}
		});
			...
			switch (selected)
			{
				...
				case ACCOUNT:
					...
					label2.setText("Target:");
					{
						com.goalplanner.model.AccountMetric m =
							(com.goalplanner.model.AccountMetric) metricCombo.getSelectedItem();
						boolean isCa = m == com.goalplanner.model.AccountMetric.CA_POINTS;
						boolean isLeaguePoints = m == com.goalplanner.model.AccountMetric.LEAGUE_POINTS;
						boolean isLeagueTasks = m == com.goalplanner.model.AccountMetric.LEAGUE_TASKS;
		...
		okButton.addActionListener(e ->
		{
			...
			else if (selectedType == GoalType.BOSS)
			{
				...
				try
				{
					...
					{
						int varpId = com.goalplanner.data.BossKillData.getVarpId(selectedBoss);
						if (varpId >= 0)
						{
